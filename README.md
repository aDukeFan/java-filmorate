...
## Изменения от 30 мая 2024
* В связи с добавлением нового функционала обновилась диаграмма базы данных:

![TablesDiagramDB](src/main/resources/TablesDiagramDB.png)
### 1. Добавлен новый функционал "Оценки" и получение рекомендаций согласно оценкам
#### Пользователи могут ставить фильмам оценки:
  * APT `POST /films/{filmId}/grade/{userId}?value=[]`
    * оценка от 1 до 5 считается отрицательной, 
    * оценка от 6 до 10 — положительной
#### Пользователи могу изменять свою оценку:
  * APT `PUT /films/{filmId}/grade/{userId}?value=[]`
#### а также удалять свою оценку:
  * APT `DELETE /films/{filmId}/grade/{userId}`

#### Появилась дополнительная система рейтинга на основе оценок пользователя: 
* рейтинг вычисляется как среднее арифметическое и представляет собой дробную величину с одним знаком после запятой.
#### Добавлена возможность выводить топ-N фильмов согласно оценке пользователей.
* Фильтрация по двум параметрам.
  * По жанру
  * За указанный год
* API `GET /films/best?count={limit}&genreId={genreId}&year={year}`
возвращает список наиболее высоко оцененных пользователями фильмов указанного жанра за нужный год.
* API `GET /films/director/{directorId}?sortBy=[grades]`
возвращает список фильмов режиссера отсортированных по оценкам пользователей

#### Добавлена возможность получать рекомендации согласно оценкам пользователя
Алгоритм работает следующим образом:
1. для пользователя (далее Клиента) происходит поиск другого пользователя 
с максимальным количеством пересечения по оценкам (далее - Рекомендующего);
2. определение фильмов, которые оценил Рекомендующий, но не оценил Клиент; 
3. Выдача списка фильмов, которым Рекомендующий поставил положительную оценку, 
а Клиент не поставил.
* `GET /users/{id}/recommendations` 
возвращает рекомендации по фильмам для просмотра.
### 2. Оптимизирован (полностью переписан) метод получения рекомендаций согласно лайкам

## Изменения от 17 мая 2024
