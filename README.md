...
1. для пользователя (далее Клиента) происходит поиск других пользователей 
с максимальным количеством пересечений по оценкам (далее - Рекомендующих);
2. определение фильмов, которые оценили Рекомендующие, но не оценил Клиент; 
3. Выдача списка фильмов, которым Рекомендующие поставили положительную оценку, 
а Клиент не поставил.
* `GET /users/{id}/best-recommendations` 
